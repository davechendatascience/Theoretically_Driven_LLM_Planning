...
def view_artifacts(ctx: "Context", subject: str | None = None) -> str:
    ...
    if written:
        lines += ["", f"written to {written} -- one line per path, sorted, with the time and the "
                  "declaration source in its header line, so committing it makes what changed a "
                  "diff rather than a memory. Regenerated by every run of this view."]
...
def _write_stamps(ctx: "Context", records) -> str:
    ...
    Beside the ledger rather than in its cache, because this one is worth committing: the
    records are sorted and carry no per-line timestamp, so a diff shows what changed verdict and
    nothing else. It stays derived -- regenerating costs a second -- and its header says when it
    was computed and from which declarations.
    """
    import json
    from datetime import datetime, timezone

    path = ctx.store.dir / "stamps.jsonl"
    path.parent.mkdir(parents=True, exist_ok=True)
    at = datetime.now(timezone.utc).isoformat(timespec="seconds")
    with path.open("w", encoding="utf-8") as fh:
        # the time and the source live in one header line, not on every record: a file meant to be
        # committed should diff on what changed, not on when it was regenerated
        fh.write(json.dumps({"kind": "header", "at": at, "declarations": ctx.decl.source,
                             "paths": len(records)}) + "\n")
